...
<form action="{{ isset($kampus) ? route('dashboard.kampus.update', $kampus->id) : route('dashboard.kampus.store') }}" method="POST" enctype="multipart/form-data">
    @csrf
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <div class="form-group">
                ...
                <small class="text-muted">Format: JPG,JPEG,PNG | Maks: 2Mb @if(isset($kampus) && $kampus->thumbnail) | <a href="{{ $kampus->thumbnail }}" target="_blank">Lihat foto</a> @endif</small>
                @error('thumbnail')
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control @error('nama') is-invalid @enderror" name="nama" id="nama" value="{{ isset($kampus) ? $kampus->nama : old('nama') }}" required>
                @error('nama')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
